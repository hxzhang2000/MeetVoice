...
### Added
- **LLM 接入 Ollama 本地大模型**（零配置、全本地零外传）：配置层 `provider` 区分
  （openai/ollama）与 `ollama_host` 字段；新增 `meetvoice/llm` 包（`build_llm_client`
  工厂 + Ollama 辅助）；CLI `llm` 命令组（check/list/pull）；设置 UI 新增「接入方式」下拉。
- **版本控制**：包与 `pyproject.toml` 统一版本号 `0.1.1`；`meetvoice/__init__.py`
...
- **软件显示版本号**：
  ...
  - 桌面端：托盘悬浮标题显示 `MeetVoice v0.1.1`；设置窗口底部显示版本与 GitHub 链接；
    新增「关于」对话框。
